...
1. 在仓库根目录执行 `./gradlew.bat :backend:bootRun`，让本地演示后端先跑起来。
2. 把 Android 工程 `local.properties` 中的 `base.url` 改成电脑当前局域网 IP，例如 `http://192.168.1.10:8080/api/`，供 Pixel 3 真机访问。

### 我下一步要做的事情

1. 跟你一起完成一次“真机连接本地后端”的同步联调闭环，确认“立即同步”按钮在设备上可用。
2. 在同步联调稳定后，继续做答辩演示路径的收口，包括结果提示、演示顺序和必要的小异常处理。
